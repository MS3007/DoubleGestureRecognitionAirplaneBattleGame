...
from gameAssetPy import five_bullets_h,obstacles_nm,initialize_spaceships_h
...
pygame.display.set_caption("手势识别太空大战")
...
def draw_window(red,reds, yellow,yellows, red_bullets, yellow_bullets, red_health, yellow_health, health_packages,tag,yellow_state,special_areas,red_state,obstacles, current_frame,yellow_skill_ready, red_skill_ready):
    """
    :param yellows:
    :param reds:
    :param red:
    ...
    red_health_text = HEALTH_FONT.render(
        "Health: " + str(round(red_health, 2)) + " State: " + str(red_state), 1, WHITE)  # ！ 左边血，状态
        # "Health: " + str(red_health), 1, WHITE)    #zby原来的
    yellow_health_text = HEALTH_FONT.render(
        "Health: " + str(round(yellow_health, 2)) + " State: " + str(yellow_state), 1, WHITE)  # ！ 右边血，状态
    ...
    for package in health_packages:  # 假如血包有元素，就画出来
        ...
        if tag == 0:
            WIN.blit(pygame.transform.scale(image_2, (20, 20)), package)
        elif tag == 1:
            WIN.blit(pygame.transform.scale(image_1, (20, 20)), package)
        elif tag == 2:
            WIN.blit(pygame.transform.scale(image_0, (20, 20)), package)
        else:
            WIN.blit(pygame.transform.scale(image_3, (20, 20)), package)
    #*
...
def gesture_movement(gestureL,gestureR,yellow, red, yellows, reds):
    ...
    if float(Lconfidence) > 0.60:
        if Llabel == "left" and yellow.x - VEL > 0:  # LEFT
            yellow.x -= VEL
            for temp in yellows:
                temp.x -= VEL
        if Llabel == "right" and yellow.x + VEL + yellow.width < BORDER.x:  # RIGHT
            yellow.x += VEL
            for temp in yellows:
                temp.x += VEL
        if Llabel == "up" and yellow.y - VEL > 0:  # UP
            yellow.y -= VEL
            for temp in yellows:
                temp.y -= VEL
        if Llabel == "down" and yellow.y + VEL + yellow.height < HEIGHT - 15:  # DOWN
            yellow.y += VEL
            for temp in yellows:
                temp.y += VEL
    if float(Rconfidence) > 0.60:
        if Rlabel == "left" and red.x - VEL > BORDER.x + BORDER.width:  # LEFT
            red.x -= VEL
            for temp in reds:
                temp.x -= VEL
        if Rlabel == "right" and red.x + VEL + red.width < WIDTH:  # RIGHT
            red.x += VEL
            for temp in reds:
                temp.x += VEL
        if Rlabel == "up" and red.y - VEL > 0:  # UP
            red.y -= VEL
            for temp in reds:
                temp.y -= VEL
        if Rlabel == "down" and red.y + VEL + red.height < HEIGHT - 15:  # DOWN
            red.y += VEL
            for temp in reds:
                temp.y += VEL
    ...
    while True:
        ...
        while run:
            ...
            draw_window(red, reds, yellow, yellows, red_bullets,
                        ...
                        red_state, obstacles, current_frame, yellow_skill_ready, red_skill_ready)
            #*
