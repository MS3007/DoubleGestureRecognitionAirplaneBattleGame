...
#yll
bvel=3
bull_WIDTH, bull_HEIGHT = 20, 40  # 设置导弹的宽度和高度。
bomb_image = pygame.image.load("./Assets/yll4.png").convert_alpha()
bomb_image = pygame.transform.scale(bomb_image, (bull_WIDTH, bull_HEIGHT))
#*
# hsz_nm
...
TECH_BOOM_SOUND = pygame.mixer.Sound('Assets/techboom.mp3')
...
def draw_window(red,reds, yellow,yellows, red_bullets, yellow_bullets, red_health, yellow_health, health_packages,tag,yellow_state,special_areas,red_state,obstacles, current_frame,yellow_skill_ready, red_skill_ready,username1,username2,bull_x, bull_y,bomb_image):
    """
    :param yellows:
    :param reds:
    :param red:
    ...
    #yll
    WIN.blit(bomb_image, (bull_x, bull_y, bull_WIDTH, bull_HEIGHT))  # 绘制导弹
    #*
...
def main():
    ...
    while True:
        ...
        #*
        #yll
        last_dd_time = time.time() + 5
        bull_x = random.randint(0, WIDTH - bull_WIDTH)  # 导弹的初始 x 坐标在游戏窗口宽度范围内随机生成
        bull_y = 0  # 计算子弹的垂直位置，使每组子弹垂直居中。
        #*
        ...
        while run:
            ...
            #yll
            # 检查导弹是否移出了窗口底部
            if bull_y + bull_HEIGHT > HEIGHT:
                if time.time() - last_dd_time > random.randint(5, 10):
                    bull_x = random.randint(0, WIDTH - bull_WIDTH)
                    bull_y = 0  # 如果导弹移出窗口，将其重新放置在顶部
                    last_dd_time = time.time()

                # 更新导弹的位置
            bull_y += bvel  # 导弹向下移动
            # 检测游戏结束的条件，并显示获胜者的信息
            # 创建导弹的矩形对象
            missile = pygame.Rect(bull_x, bull_y, bull_WIDTH, bull_HEIGHT)

            # 检测导弹是否与红色飞船碰撞
            if red.colliderect(missile):
                TECH_BOOM_SOUND.play()
                if red_health == 1:
                    red_health -= 0
                else:
                    red_health -= 1  # 减少红色飞船的生命值
                BULLET_HIT_SOUND.play()
                bull_y = HEIGHT + 10
                if time.time() - last_dd_time > random.randint(5, 10):
                    bull_x = random.randint(0, WIDTH - bull_WIDTH)
            ...
            draw_window(red, reds, yellow, yellows, red_bullets,
                        ...
                        red_state, obstacles, current_frame, yellow_skill_ready, red_skill_ready,username1,username2, bull_x, bull_y,bomb_image)
            #*
